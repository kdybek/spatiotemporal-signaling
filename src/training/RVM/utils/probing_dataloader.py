...
def get_clip(root, video_name, clip_frames, clip_size, acq_freq, channel_names_list, random_crop, validation=False):
    ...
    if random_crop:
        start_t1 = np.random.randint(0, T - min_frames_needed + 1)
        start_h1 = np.random.randint(0, H - clip_size + 1)
        start_w1 = np.random.randint(0, W - clip_size + 1)
        start_t2 = np.random.randint(0, T - min_frames_needed + 1)
        start_h2 = np.random.randint(0, H - clip_size + 1)
        start_w2 = np.random.randint(0, W - clip_size + 1)
    else:
        raise NotImplementedError(
            "Currently only random_crop is implemented. Please set random_crop=True.")

    start_t1 = snap_to_chunk_size(start_t1, DATASET_T_CHUNK)
    start_h1 = snap_to_chunk_size(start_h1, DATASET_H_CHUNK)
    start_w1 = snap_to_chunk_size(start_w1, DATASET_W_CHUNK)
    start_t2 = snap_to_chunk_size(start_t2, DATASET_T_CHUNK)
    start_h2 = snap_to_chunk_size(start_h2, DATASET_H_CHUNK)
    start_w2 = snap_to_chunk_size(start_w2, DATASET_W_CHUNK)

    while start_t1 == start_t2:
        start_t2 = np.random.randint(0, T - min_frames_needed + 1)
        start_t2 = snap_to_chunk_size(start_t2, DATASET_T_CHUNK)

    clip = root[video_name][start_t1:start_t1 + min_frames_needed:step, channel_indices,
                            start_h1:start_h1 + clip_size, start_w1:start_w1 + clip_size]
    clip_same_loc_t2 = root[video_name][start_t2:start_t2 + min_frames_needed:step, channel_indices,
                                        start_h1:start_h1 + clip_size, start_w1:start_w1 + clip_size]
    clip_diff_loc_t2 = root[video_name][start_t2:start_t2 + min_frames_needed:step, channel_indices,
                                        start_h2:start_h2 + clip_size, start_w2:start_w2 + clip_size]

    if downsample_2x:
        clip = downsample_video_2x(clip)
        clip_same_loc_t2 = downsample_video_2x(clip_same_loc_t2)
        clip_diff_loc_t2 = downsample_video_2x(clip_diff_loc_t2)
...
class ZarrVideoDataset():
    ...
    def __getitem__(self, idx):
        ...
        video = vid_dict["clip"]
        vid_same_loc_t2 = vid_dict["clip_same_loc_t2"]
        vid_diff_loc_t2 = vid_dict["clip_diff_loc_t2"]
        t1 = vid_dict["t1"]
        t2 = vid_dict["t2"]

        video = video.astype("float32")
        vid_same_loc_t2 = vid_same_loc_t2.astype("float32")
        vid_diff_loc_t2 = vid_diff_loc_t2.astype("float32")
        video = self.transform_pipeline(video)
        vid_same_loc_t2 = self.transform_pipeline(vid_same_loc_t2)
        vid_diff_loc_t2 = self.transform_pipeline(vid_diff_loc_t2)
        ...
        return video, vid_same_loc_t2, vid_diff_loc_t2, exp_name, metadata
